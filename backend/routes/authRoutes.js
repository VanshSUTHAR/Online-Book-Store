...
router.post("/send-otp", async (req, res) => {
  try {
    ...
    // 1. Send HTTP response to browser first
    res.json({
      success: true,
    ...
    // 2. Dispatch email asynchronously in background safely without blocking or risking headersSent error
    setImmediate(() => {
      try {
        ...
        console.error("[OTP] Background task error:", bgError.message);
      }
    });
  } catch (error) {
